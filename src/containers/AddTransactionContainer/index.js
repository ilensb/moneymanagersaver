...
import { CategoryListing, Form, TopBar } from '../../components';
...
class AddTransactionContainer extends Component {
  ...
  componentDidAppear() {
    ...
    if (this.checkEditData()) {
      ...
      this.setState({
        ...
        currentActiveMenu:
          this.props.selectedTransactionItemData.transaction_type,
      });
    }
  }
  ...
  async fetchData(type) {
    try {
      const currentActiveMenuData = await fetchCategoryData(
        db,
        (vars = [type])
      );
  ...
  insertData = async () => {
    ...
    const updateVars = [
      ...
      amount,
      memo,
      this.props.selectedTransactionItemData.id,
    ];
    if (this.checkEditData()) {
  ...
  onClickMenu(type) {
    this.setState(
      { currentActiveMenu: type, isCategoryModalOpen: false, open: false },
      () => {
    ...
    >
      <View style={styles.categoryModalview}>
        ...
        >
          ...
          <View style={{ flex: 2 }}>{isActive('Expense') && icon}</View>
        </TouchableOpacity>
        ...
        >
          ...
          <View style={{ flex: 2 }}>{isActive('Income') && icon}</View>
        </TouchableOpacity>
      </View>
    </Modal>
  );
...
export default connect(
  (state) => ({
    selectedTransactionItemData: state.transaction.selectedItemData,
  }),
  { editTransaction }
