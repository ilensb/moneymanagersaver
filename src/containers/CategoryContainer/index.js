...
import { EmptyDataWithButton, TopBar } from '../../components';
...
class CategoryContainer extends Component {
  ...
  async fetchCategoryData() {
    const eachCategoryData = await fetchEachCategoryData(
      db,
      (vars = [this.state.selectedCategory])
    );
  ...
  renderItems = ({ name, id }) => {
    ...
    color[id] = exactColor;
    return (
      <TouchableOpacity onPress={() => this.onCategoryItemClick(name, id)}>
        <View
          style={{
            flex: 1,
            flexDirection: 'row',
        ...
        >
          ...
          >
            ...
            >
              <Image source={Images[name]} style={{ width: 20, height: 20 }} />
            </View>
          </View>
          <View style={{ flex: 5 }}>
...
function CategoryList({ categoryData, renderItems, categoryFormText }) {
  return (
...
function CategoryHeader({ onChangeCategoryText, categoryFormText }) {
  return (
    <LinearGradient
        ...
        <View style={{ paddingHorizontal: 15 }}>
          <VictoryChart minDomain={{ y: 0 }}>
            ...
            <VictoryLine
              ...
              data={
                eachCategoryData.length === 1
                  ? [eachCategoryData[0], eachCategoryData[0]]
                  : eachCategoryData
              }
            />
          </VictoryChart>
